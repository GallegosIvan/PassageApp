...
class _AiChatScreenState extends State<AiChatScreen> {
  ...
  Future<void> _loadInitialData() async {
    ...
    final tokenUsage = results[1] as Map<String, dynamic>;
  ...
  Future<void> _sendMessage() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        // Streaming done — commit final message to list
        if (mounted) {
          setState(() {
            _isStreaming = false;
            _streamingContent = '';
  ...
  Future<void> _animateText(String fullText) async {
    // Target: always finish in ~2 seconds regardless of length
    const targetDuration = 2000; // ms
    const minDelay = 4;          // ms per chunk (fastest)
    const maxDelay = 20;         // ms per chunk (slowest)
    const chunkSize = 3;
    ...
    if (mounted) {
      setState(() => _streamingContent = fullText);
    }
  }
  ...
  void _showLimitDialog(AiLimitException e) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text(
          'You\'ve used ${e.tokensUsed} of ${e.dailyLimit} tokens today. Your limit resets at midnight.\n\nUpgrade to Pro for more daily tokens.',
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      // Streaming bubble (animating text)
                      ...
                      Expanded(
                        child: TextField(
                          ...
                          decoration: InputDecoration(
                            hintText: 'Ask about a verse or topic...',
